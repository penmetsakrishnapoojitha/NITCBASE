  ...
  ************************/
  ...
  while (true) {
    ...
    if (searchRes.block != -1 && searchRes.slot != -1) {
      ...
      // FIX 3: Renamed BlockBuffer to recBuffer
      RecBuffer recBuffer(searchRes.block); 
      HeadInfo blockHeader;
      recBuffer.getHeader(&blockHeader);

      Attribute recordBuffer[blockHeader.numAttrs];
      recBuffer.getRecord(recordBuffer, searchRes.slot);

      // FIX 1: Add the leading '|' to match the header border
      printf("|"); 
      ...
      for (int i = 0; i < relCatEntry.numAttrs; ++i) {
        ...
        if (attrCatEntry.attrType == NUMBER) {
          // FIX 2: removed (int) cast and changed %d to %f
          printf(" %f |", recordBuffer[i].nVal); 
        } else {
